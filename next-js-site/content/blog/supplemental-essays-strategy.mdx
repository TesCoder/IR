...
Start here if you already read the hub: [Supplemental Essays by School Type](/blog/supplements-by-school-type-hub) is the routing map; this page is the execution playbook for sequencing, reuse, and proof.
...
| School type | What to emphasize first | Biggest trap to avoid |
|---|---|---|
| Ivy+ / selective privates | Intellectual depth + specific academic plan | Generic prestige praise ("world-class faculty") |
| Publics + honors | Program access + what you'll build with it | Treating it like a backup (undercooked specificity) |
| Special programs (BS/MD, accelerated) | Long-term commitment with sustained evidence | Overpromising impact without a history to back it up |
| Short-answer-heavy schools | Voice + modular proof blocks per question | Cloning answers with a name swap |
...
| Asset | Example entry |
|---|---|
| Proof point | Co-built peer translation network; trained 6 volunteers; served 30+ families during FAFSA season; ran for 2 consecutive application cycles. |
| Theme | Systems builder who creates tools that outlast her |
| Mini anecdote | The afternoon she watched three families leave without submitting — because the form was English-only |
| Fit thesis (reach school) | "At [School], I want to use [public policy lab] to scale community-access models built on my translation work — and [first-gen org] to keep the practice going." |
